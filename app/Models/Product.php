...
{
    protected $fillable = ['category_id', 'set_id', 'promotion_id', 'alias', 'on_home', 'position', 'price', 'actual_price', 'price_lei', 'actual_price_lei', 'discount', 'hit', 'recomended', 'stock', 'code', 'pdf', 'discount_update'];

    protected $appends = ['url'];

    public function getUrlAttribute() {
        if(count($this->category->parent) > 0) {
          return 'catalog/'.$this->category->parent->alias.'/'.$this->category->alias.'/'.$this->alias;
        } else {
    ...
    {
        return $this->hasOne(ProductImage::class, 'product_id')->where('type', 'image')->orderBy('id', 'desc');
    }
    ...
    {
         return $this->hasMany(ProductImage::class, 'product_id')->where('type', 'image')->orderBy('id', 'desc');
    }

    public function lifestyleImages() {
        return $this->hasMany(ProductImage::class, 'product_id')->where('type', 'lifestyleImage')->orderBy('id', 'desc');
    }
    ...
    public function setProduct() {
        return $this->hasOne(SetProducts::class);
    }
}
